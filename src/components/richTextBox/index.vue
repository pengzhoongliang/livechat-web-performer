<template>
    <div>
        <quill-editor v-model="content"
                     ...
                     @change="onChange"
        >
            <div :id="'toolbar'+sizeType" slot="toolbar">
...
<script>
    ...
    // import { uploadImage } from '@/api/table'
    ...
    export default {
        ...
        methods: {
            ...
            onFileChange(e){
              ...
              xhr.send(data);
              xhr.onload=function () {
                if(xhr.status==200&&xhr.response.code==200){
                  let path=process.env.VUE_APP_URL + process.env.VUE_APP_BASE_API+"/resource/"+xhr.response.data.path;
                  console.log("全拼接："+path)
                  let img = '<img src="'+path+'">'

                  //let img = '<img src="http://127.0.0.1:8078/dev-api'+xhr.response.data.path+'">'
                  self.content += img;
